  ...
  .handler(async ({ context }) => {
    ...
    const alliesRows = allyRows.flatMap((a) => {
      ...
      const base = {
        ...
        "Tipo de acuerdo": a.agreement_type ?? "",
        "Tipo de aliado": a.partner_type ?? "",
        "Función misional": a.mission_function ?? "",
        "Valor compartido": a.shared_value ?? "",
        Condiciones: a.conditions ?? "",
        "Aplica para": a.applies_to ?? "",
        "Fecha de cierre": a.close_date ?? "",
        "Valor económico": a.economic_value ?? "",
        "Dependencia UAM que genera": a.dependency_origin ?? "",
        "Dependencia UAM que implementa": a.dependency_implementer ?? "",
