-- ============================================================================
-- Indicadores estratégicos oficiales — Direccionamiento Estratégico UAM 2024-2030
-- Copiar este archivo a supabase/migrations/ manteniendo el prefijo de fecha.
--
-- Principio de diseño: los 10 indicadores y sus metas NO se calculan ni se
-- inventan aquí; se cargan tal cual aparecen en el PDF (pág. 56-58) en la
-- tabla de referencia `strategic_indicators`, para que la app siempre lea
-- la meta oficial en vez de tener el número hardcodeado en el frontend.
-- ============================================================================

-- 1) Catálogo de indicadores estratégicos oficiales (fuente única de verdad)
-- Nota: direction_hint reutiliza como texto libre el mismo dominio de valores
-- que ya usa public.allies.direction (alianzas, investigacion, decanaturas,
-- proyeccion, relaciones_internacionales) en vez de crear un tipo nuevo, para
-- no duplicar el CHECK que ya vive en la tabla allies (migración
-- 20260708132132...). Es NULL para los indicadores del OE4, que no
-- corresponden a ninguna dirección de la app (ver sección 4 del documento
-- de análisis adjunto).
CREATE TABLE IF NOT EXISTS public.strategic_indicators (
  key text PRIMARY KEY,                 -- identificador estable, usado por la app
  objetivo text NOT NULL,               -- 'OE1'..'OE4'
  programa text NOT NULL,               -- nombre literal del programa (pág. 25-56)
  label text NOT NULL,                  -- texto literal del indicador (pág. 56)
  unit text NOT NULL,                   -- 'numero' | 'moneda_cop' | 'porcentaje'
  meta_2030 numeric,                    -- meta final, NULL si el documento no da un número único
  meta_2030_nota text,                  -- para casos con inconsistencia en el PDF (ver indicador 6)
...
-- Solo admins pueden editar el catálogo (evita que alguien "invente" una meta nueva desde la UI)
CREATE POLICY "strategic_indicators_admin_manage" ON public.strategic_indicators
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::public.app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::public.app_role));

-- 2) Carga de los 10 indicadores oficiales (literal del PDF, pág. 56-58)
...
-- 3) Metas anuales (progresión 2024-2030) — tabla separada porque el documento
--    da una meta por año, no solo la meta final (pág. 57-58).
...
-- Nota: el salto de 4 a 2 en 'indice_huella_ambiental' (2028->2029) es tal cual
-- aparece en el gráfico de la pág. 58 del PDF; no es un error de esta migración.

-- 4) Aporte de cada aliado a un indicador (esto es lo que se llena desde AllyDialog.tsx)
CREATE TABLE IF NOT EXISTS public.ally_indicator_contributions (
  ...
  proyecto_estrategico text,              -- texto literal del proyecto (ej. 'Trayectorias Vitales de Aprendizaje')
...
-- Mismas reglas de acceso que ya usa la tabla allies: admin, dueño del aliado (por dirección) o creador
...
-- 5) Vista de apoyo: total aportado por dirección/indicador/año vs. meta oficial del PDF
