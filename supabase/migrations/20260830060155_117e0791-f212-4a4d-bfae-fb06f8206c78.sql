...
DROP POLICY IF EXISTS "user_indicator_profiles_select_self" ON public.user_indicator_profiles;
CREATE POLICY "user_indicator_profiles_select_self" ON public.user_indicator_profiles
  FOR SELECT TO authenticated
  USING (user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'::public.app_role));

DROP POLICY IF EXISTS "user_indicator_profiles_admin_manage" ON public.user_indicator_profiles;
